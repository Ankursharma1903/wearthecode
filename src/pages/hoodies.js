      ...
      <section className="text-gray-600 body-font">
        <div className="container px-5 py-24 mx-auto">
          <div className="flex flex-wrap -m-4 justify-center">
            {Object.keys(products).length ===0 && <p>Sorry all hoodies are currently out of stock. New stock coming soon. Please stay tuned</p>}
...
let products=await Product.find({category:'hoodies'})// if find() without category so show every product but to get by category we mention categoryy in this
// so other products than this should not display on hoods page
...
// as products is array and need to iterate through it
   ...
   if(item.title in hoods){
    // here key value pair is made and in hoods the key is title
    ...
    // title already present so need to check if that new variant for color and size is already present or not
      ...
      hoods[item.title].color.push(item.color)             // so pushed in array item.color
        }
        if(!hoods[item.title].size.includes(item.size) && item.availableQty>0){
          // so if size not present
      hoods[item.title].size.push(item.size)             // so pushed in array item.size
        }


   }else{
    // if not present so check the available qty>0 so made arrayy of color and size
    hoods[item.title]=JSON.parse(JSON.stringify(item))       // pare and stringify together means deep copy
    if(item.availableQty > 0){
      hoods[item.title].color=[item.color] // so made an array of color hoods[item.title].color
  ...
  return{
    ...
    // here after returning hoods instead of products so products now its not an array now its an object so need to make changes in map function
